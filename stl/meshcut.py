...
class TriangleMesh(object):
    def __init__(self, verts, tris):
        ...
        for tid, f in enumerate(tris):
            tri_edges = []
        # Loop over all of the points in the triangle and
        # determine all 3 edges (that is what the % 3 does below
            for i in range(3):
                v1 = f[i]
                v2 = f[(i + 1) % 3]
                e = make_edge(v1, v2)
                # This bit of code gathers up all of the triangles that share an edge
                # BTW: We need to understand what the collections.defaultdict(lambda: []) does
                # edges_to_tris will provide the triangles when given an edge
                self.edges_to_tris[e].append(tid)
                # Gather up the edges for this triangle
                tri_edges.append(e)
                # verts_to_tris will provide the triangle when given the vertex
                self.verts_to_tris[f[i]].append(tid)
            # tris_to_edges returns the edges when given the triangle
            self.tris_to_edges[tid] = tri_edges

        # The models we are looking are large and problems with the model
        # may not be in the area of interest.  Lets forget the assert for
        # now.  
        # Sanity check : max 2 faces per edge
        #for e, triangles in self.edges_to_tris.items():
        #    if len(triangles) > 2:
        #        print("-------")
        #        print(e,' ',triangles)
        #        for tid in triangles:
        #            print(tid,': ',tris[tid,0],' ',tris[tid,1],' ',tris[tid,2])
        #            for j in [0,1,2]:
        #               print("    ",tris[tid,j]," ",verts[tris[tid,j]])
        #     assert len(tris) <= 2
...
def compute_triangle_plane_intersections(mesh, tid, plane, dist_tol=1e-8):
    ...
    # Below is a dictionary list comprehension.
    # Calculates the distance between eacy point in the triangle
    # and the plane
...
def merge_close_vertices(verts, faces, close_epsilon=1e-5):
    ...
    # The length is number of points in the verts array.
...
def merge_close_vertices_big(verts, faces, close_epsilon=1e-5):
    """
    Will merge vertices that are closer than close_epsilon.

    The original merge_close_vertice calculated all of the distances
    at once which led to an N^2 allocation which doesn't work for large
    meshes   Rewrite code to calculate the distances line by line.

    Returns: new_verts, new_faces
    """
# We are NOT doing this N^2 allocation.   It will kill us    
#    # Pairwise distance between verts
#    if USE_SCIPY:
#        D = spdist.cdist(verts, verts)
#    else:
#        D = np.sqrt(np.abs(pdist_squareformed_numpy(verts)))
    ...
    # Compute a mapping from old to new : for each input vert, store the index
    # of the new vert it will be merged into
    old2new = np.zeros(verts.shape[0], dtype=np.int)
    old2newsorted = np.zeros(verts.shape[0], dtype=np.int)
    # A mask indicating if a vertex has already been merged into another
    merged_verts = np.zeros(verts.shape[0], dtype=np.bool)
    new_verts = []

# Determine the radius distance from the origin of all of the points
#
    origin = np.array([[0,0,0]])
    realfar = np.array([[10000000,10000000,10000000]])
    radius = spdist.cdist(origin,verts)
# Sorted Index will have the indices of the vertices sorted
# by distance from the origin.

    sorted_index = np.argsort(radius)
#
# Generate two arrays that have the number of vertices that need to
# have their point to point distances calculated to see if there is
# overlap
#
    
    radius_difference = []
    for i in range(sorted_index.shape[1]-1):
        radius_difference.append(radius[0,sorted_index[0,i+1]]-radius[0,sorted_index[0,i]])
    nprad = np.array(radius_difference)
    if close_epsilon == 0.0:
        milestonepoints_np = np.flatnonzero(nprad > 1e-05 )
    else:
        milestonepoints_np = np.flatnonzero(nprad > 10*close_epsilon )
    milestonepoints = milestonepoints_np.tolist()
    milestonedifference = []
    milestonedifference.append(milestonepoints[0])
    for i in range(len(milestonepoints)-1):
        milestonedifference.append(milestonepoints[i+1]-milestonepoints[i])
# Append the final point to the milestonedifference and milestonepoints list

    lastmilestone = milestonepoints[len(milestonepoints)-1]
    milestonedifference.append(verts.shape[0]-lastmilestone)
    milestonepoints.append(verts.shape[0])

        
# So, any point that is between two entries between the milestone points only needs                    
# to consider the number of points that is in milestonedifference.  

# Create a local array to bring together points that are close to minimize
# the calculation time
#    localvertlength = 3000
#    localverts = np.zeros((localvertlength,3),dtype=np.float)
# Initialize the milestoneindex to the beginning of the arrays

    milestoneindex = 0
    
# First, calculate the indexes for the radius
    for i in range(verts.shape[0]):
        if (i % 50000 == 0):
            percent = str(100*i/verts.shape[0])[0:4]
            print("Currently at :",i,"/",verts.shape[0]," ",percent,"% ",datetime.now())
        if merged_verts[i]:
            continue
        else:
    # We are going to pull the points from the sorted_index
            current_point = np.array([verts[sorted_index[0,i]]])
    # Now, lets see how many points that we need to calculate the point
    # to point distance.
#            print(i," ",milestoneindex," ",milestonepoints[milestoneindex]," ",milestonedifference[milestoneindex]," ",len(milestonedifference)," ",len(milestonepoints)," ",verts.shape[0])
            if i < milestonepoints[milestoneindex]:
                localvertlength = 2*milestonedifference[milestoneindex]
            else:
                milestoneindex = milestoneindex + 1
                localvertlength = 2*milestonedifference[milestoneindex]
            localverts = np.zeros((localvertlength,3),dtype=np.float)    
            for j in range(0,localvertlength):
                if (i+j) < verts.shape[0]:
                   localverts[j] = verts[sorted_index[0,i+j]]
                else:
                    localverts[j] = realfar
            D = spdist.cdist(current_point, localverts)
#           localverts = None
            # The vertices that will be merged into this one
            localmerged = np.flatnonzero(D[:] <= close_epsilon)
            mergedlist = []
            for j in range(len(localmerged)):
                 mergedlist.append(localmerged[j])
            merged = np.array(mergedlist) + i
            old2newsorted[merged] = len(new_verts)
            new_verts.append(current_point)
            merged_verts[merged] = True

    # At this point old2newsorted contains the indexes into the new_verts array as sorted
            
    new_verts = np.array(new_verts)

    # Reorder back into the original way the points came in.
    for i in range(old2newsorted.shape[0]):
        old2new[sorted_index[0,i]] = old2newsorted[i]
