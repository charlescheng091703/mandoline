#!/usr/bin/env python
"""
Script takes the procressed triangles 
from an STL file and determines intersections
...
def limit_faces(index, vert, faces, cut_bins_db, query):
    ...
    Args:
        ...
        cut_bins_db (_type_): _description_
...
def perform_mesh(fileprefix, axis, interval, offsetfile):
    ...
    Args:
        fileprefix (str): The file prefix for this calculation.  Used to calculate standard names.
        axis (str): The axis to perform the slicing (X,Y,Z)
        interval (int): Determines the interval at which the mesh is sampled (normally this will be 1)
        offsetfile (str): The name of an optional offset file to shift the slicing coordinate to help manage singularities.
    """
    ...
    if axis == "X":
        ...
        for row in cut_bins_db.query(
            "select value from variables where name = 'Xmin';"
        ):
            start = row["value"]
        for row in cut_bins_db.query(
            "select value from variables where name = 'Xmax';"
        ):
            stop = row["value"]
    ...
    if axis == "Y":
        ...
        for row in cut_bins_db.query("select value from variables where name = 'yn';"):
            bin_multiplier = row["value"]
        for row in cut_bins_db.query(
            "select value from variables where name = 'Ymin';"
        ):
            start = row["value"]
        for row in cut_bins_db.query(
            "select value from variables where name = 'Ymax';"
        ):
            stop = row["value"]
    ...
    if axis == "Z":
        ...
        for row in cut_bins_db.query("select value from variables where name = 'zn';"):
            bin_multiplier = row["value"]
        for row in cut_bins_db.query(
            "select value from variables where name = 'Zmin';"
        ):
            start = row["value"]
        for row in cut_bins_db.query(
            "select value from variables where name = 'Zmax';"
        ):
            stop = row["value"]
    ...
    for index in indices:
        ...
        if sortaxis == 0:
            query = (
                "select triangle_index from X where slice_index = " + str(index) + ";"
            )
        if sortaxis == 1:
            query = (
                "select triangle_index from Y where slice_index = " + str(index) + ";"
            )
        if sortaxis == 2:
            query = (
                "select triangle_index from Z where slice_index = " + str(index) + ";"
            )
        ...
        xcoords, ycoords, zcoords, intersections = get_all_intersections(
            mesh, plane, index / bin_multiplier
        )
        print("Output Intersections")
        db_table.insert_all(intersections)
        db_offset_table.insert(
            {"slice_value": slice_value, "slice_offset": slice_offset}
        )
